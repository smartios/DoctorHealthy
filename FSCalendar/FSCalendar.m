...
{   
    ...
    _scope = FSCalendarScopeMonth;
     selectedDates = [NSMutableArray arrayWithCapacity:100];
     BookedDates = [NSMutableArray arrayWithCapacity:100];
    ...
    _placeholderType = FSCalendarPlaceholderTypeNone;
   ...
   // contentView.layer.borderColor = [UIColor colorWithRed:137.0/255.0f green:217.0/255.0f blue:210.0/255.0f alpha:1.0f].CGColor;
    [self addSubview:contentView];
    self.contentView = contentView;
    
     //UIButton *rightArrow = [[UIView alloc] initWithFrame:CGRectZero];
    
    UIView *daysContainer = [[UIView alloc] initWithFrame:CGRectZero];
    daysContainer.backgroundColor = [UIColor clearColor];
   // daysContainer.layer.borderWidth = 2.0;
   // daysContainer.layer.borderColor = [UIColor colorWithRed:137.0/255.0f green:217.0/255.0f blue:210.0/255.0f alpha:1.0f].CGColor;
...
- (void)prepareForInterfaceBuilder
{
    ...
    [selectedDates addObject:[self.gregorian dateFromComponents:components]];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(self.allowMultipleSelect == NO)
    {
    ...
    imageView.image =   [UIImage imageNamed:@"availabilityC"];
    cell.image = [UIImage imageNamed:@"availabilityC"];
    
    for (int i = 0; i < _BookedDates.count; i++)
    {
        
        NSString *dateFromServer = [self getDateStringFromDateString:[[_BookedDates objectAtIndex:i] valueForKey:@"available_date"] fromDateString:@"yyyy-MM-dd" toDateString:@"yyyy-MM-dd"];
        
        NSString *d = [self getDateStringFromDate:date toDateString:@"yyyy-MM-dd"];
        
        
        if ([dateFromServer isEqualToString:d])
        {
            imageView.hidden = false;
            
            if ([[[_BookedDates objectAtIndex:i] valueForKey:@"slot_status"] isEqualToString:@"booked"])
            {
...
//                }
                ...
                //cell.titleLabel.textColor = [UIColor redColor];
                 ...
                 cell.titleLabel.textColor = cell.preferredTitleSelectionColor ?: [cell colorForCurrentStateInDictionary:_appearance.titleColors];
                //cell.titleLabel.textColor = [cell preferredTitleSelectionColor];
            }
            else
            {
                cell.selected = false;
...
//    if ([self.calculator dateForIndexPath:indexPath] == selectedDate)
//    {
//        selectedDate = [[NSDate alloc] init];
//    }
//    else
//    {
     selectedDate = [self.calculator dateForIndexPath:indexPath];
 //   }
    
    NSString *dateString = [self getDateStringFromDate:selectedDate toDateString:@"dd-MMM-yyyy"];
    
    NSMutableArray *localBookedDatesArray = [[NSMutableArray alloc] init];
    
    for (int i = 0; i < _BookedDates.count; i++)
    {
        
        [localBookedDatesArray addObject:[[_BookedDates objectAtIndex:i] valueForKey:@"available_date"]];
    }
    ...
    if (![selectedDates containsObject:selectedDate])
    {
        //cell.selected = YES;
        //change
    ...
    else if (![localBookedDatesArray containsObject:dateString])
    {
           cell.selected = NO;
            cell.image = nil;
            ...
            [selectedDates removeObject:selectedDate];
...
- (NSDate *)selectedDate
{
    return selectedDates.lastObject;
}

- (NSArray *)selectedDates
{
    return [NSArray arrayWithArray:selectedDates];
}
...
- (BOOL)isDateSelected:(NSDate *)date
{
    return [selectedDates containsObject:date] || [_collectionView.indexPathsForSelectedItems containsObject:[self.calculator indexPathForDate:date]];
}
...
- (void)reloadDataForCell:(FSCalendarCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.selected = [selectedDates containsObject:date];
...
- (void)setNeedsConfigureAppearance
{
    ...
    - (NSString *) getDateStringFromDateString :(NSString *)date fromDateString:(NSString*)fromDate toDateString: (NSString*)toDate
        {
...
- (NSString *) getDateStringFromDate :(NSDate *)date toDateString: (NSString*)toDate
{
